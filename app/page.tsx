...
function EcosystemShard({
  ...
  index: number;
  shardDistance: MotionValue<number>;
  shardFade: MotionValue<number>;
}) {
  const shardX = useTransform(shardDistance, (value) => direction.x * value);
  const shardY = useTransform(shardDistance, (value) => direction.y * value);
  const shardRotate = useTransform(shardDistance, (value) => (direction.x + direction.y) * 0.14 * value);
  ...
  return (
    <main className="relative overflow-x-clip bg-[#0000FF] text-white">
      ...
      <section id="about" className="relative overflow-hidden pb-14 pt-8 text-white [perspective:2200px] md:pb-20 md:pt-12">
        ...
        <div className="container relative z-10 mx-auto grid items-center gap-10 px-4 sm:px-6 md:gap-16 lg:grid-cols-[0.95fr_1.05fr]">
          <motion.div
            initial={{ opacity: 0, y: 40, rotateX: 12 }}
            whileInView={{ opacity: 1, y: 0, rotateX: 0 }}
            viewport={{ once: true, amount: 0.2 }}
            transition={{ duration: 0.8, ease: "easeOut" }}
            className="relative overflow-hidden rounded-[1.5rem] border border-white/12 bg-white/8 p-5 shadow-[0_30px_120px_rgba(0,0,0,0.24)] backdrop-blur-xl [transform-style:preserve-3d] sm:p-8 md:rounded-[2rem] md:p-10"
          >
            ...
            >
              ...
              <div className="relative flex h-full flex-col justify-between [transform:translateZ(36px)]">
                <div>
                  ...
                  <h3 className="mt-4 text-2xl font-bold sm:text-4xl">Complete Business Technology Stack</h3>
            ...
            >
              ...
              >
                ...
                {([
                  ...
                  { x: 0, y: 1 },
                  { x: 1, y: 1 },
                ] as ShardDirection[]).map((direction, index) => (
                  <EcosystemShard
                    key={`${direction.x}-${direction.y}`}
                    direction={direction}
                    index={index}
                    shardDistance={shardDistance}
                    shardFade={shardFade}
                  />
                ))}
